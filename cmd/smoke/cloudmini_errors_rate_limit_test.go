...
func TestRunCloudminiErrorEvidenceWithRateLimitFixture(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/api/v3/error-fixtures/rate-limited":
			fixtureCalls++
			if r.Header.Get("Authorization") != "Bearer secret-token" {
				t.Fatalf("expected valid auth header")
			}
	...
	t.Setenv("CLOUDMINI_ERROR_EVIDENCE_ALLOW_RATE_LIMITED", "yes")
	t.Setenv("CLOUDMINI_ERROR_EVIDENCE_RATE_LIMIT_APPROVED", "yes")
	t.Setenv("CLOUDMINI_ERROR_EVIDENCE_RATE_LIMIT_MAX_REQUESTS", "1")
	t.Setenv("CLOUDMINI_ERROR_EVIDENCE_RATE_LIMIT_FIXTURE_PATH", "/api/v3/error-fixtures/rate-limited")
	...
	for _, leaked := range []string{"secret-token", "should-not-leak", "proxy-raw-secret", "fixture rate limited secret detail"} {
...
func TestCloudminiErrorEvidenceRequiresRateLimitFixtureApproval(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "CLOUDMINI_ERROR_EVIDENCE_RATE_LIMIT_APPROVED") {
		t.Fatalf("expected rate-limit approval error, got %v", err)
	}
}
