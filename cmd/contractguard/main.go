...
func trackedBillingContracts() []routeContract {
	...
	return []routeContract{
		contract("GET", "/admin/catalog/provider-readiness", "catalog.view", []string{"product_type", "status", "limit", "cursor"},
		...
		contract("POST", "/client/orders", "order.create", nil, []string{"Idempotency-Key", "`tenant_plan_id`"},
			source("internal/modules/order/http_handler.go", `"/client/orders"`),
			source("cmd/api/main.go", "PermissionOrderCreate")),
		contract("GET", "/admin/orders", "order.view", []string{"buyer_user_id", "display_id", "status", "billing_status", "amount_min", "amount_max", "limit", "cursor"}, nil,
		...
		contract("GET", "/admin/services", "service.view", []string{"buyer_user_id", "display_id", "order_id", "order_display_id", "status", "limit", "cursor"}, nil,
			source("internal/modules/order/http_handler.go", `"/admin/services"`),
			source("cmd/api/main.go", "PermissionServiceView")),

		contract("GET", "/client/invoices", "wallet.view", []string{"display_id", "order_id", "status", "amount_min", "amount_max", "limit", "cursor"}, nil,
			source("internal/modules/invoice/http_handler.go", `"/client/invoices"`),
			source("cmd/api/main.go", "PermissionWalletView")),
		contract("GET", "/admin/invoices", "wallet.view", []string{"buyer_user_id", "display_id", "order_id", "status", "amount_min", "amount_max", "limit", "cursor"}, nil,
		...
		contract("GET", "/client/transactions", "wallet.view", []string{"display_id", "order_id", "invoice_id", "type", "status", "amount_min", "amount_max", "limit", "cursor"}, nil,
		...
		contract("GET", "/admin/jobs", "order.view", []string{"display_id", "job_type", "status", "reference_type", "reference_id", "source_id", "limit", "cursor"}, []string{"`payload_json`", "`idempotency_key`"},
		...
		contract("POST", "/admin/jobs/{job_id}/cancel", "provisioning.manual_review.resolve", nil, []string{"`reason`", "`job.status_conflict`"},
			...
			source("cmd/api/main.go", "PermissionManualReviewResolve")),
		contract("GET", "/reseller/jobs", "order.view", []string{"display_id", "job_type", "status", "reference_type", "reference_id", "source_id", "limit", "cursor"}, []string{"tenant scope"},
