...
| Target-environment verification | Pass for completed local target dev/test evidence with T245 sign-off, but blocked for domain auth/RBAC evidence. T230-T254 prove target deploy/build, health checks, non-mutating registry activation, one Cloudmini lifecycle cleanup activation, target top-up review, local API auth/RBAC, credential reveal audit/redaction, balanced finance reconciliation, cloudflared token-file handling, and selected-host secret-store proof. T265 attempted `dev-target-auth-rbac` against `https://billing.resvn.net/backend` and was blocked because target auth login returned HTTP `500` before auth assertions. | Fix the approved domain/backend route, rerun domain `dev-target-auth-rbac`, and repeat T254 secret-store proof for any new host/path. | Security Owner, Finance Lead, QA Lead |
...
T244 manual fallback drill review passed; T264 selects manual fallback as the notification path for the selected bounded pilot; sampled customer-facing top-up approved event from T235 and ops-facing Cloudmini manual-review event from T232.
...
T265 domain check:
BLOCKED. `APP_ENV=dev go run ./cmd/smoke -base-url https://billing.resvn.net/backend -timeout 20s dev-target-auth-rbac` failed before auth assertions with `target auth login expected HTTP 200, got 500`. Root and `/backend/healthz` checks on the same domain also returned HTTP `500` from the current target route. No raw cookies, session tokens, passwords, DSNs, provider payloads, or credentials were printed. Fix the approved domain/backend route and rerun the smoke before GO.
...
Domain cross-tenant negative check: T265 did not reach this assertion because the domain auth login returned HTTP `500`.
...
Security sign-off:
Admin for T237/T240/T247 target dev/test evidence, T244/T264 selected-pilot manual fallback scope, and T254 selected-host secret-store proof; repeat the proof for any new host/path before launch use.
Finance sign-off:
