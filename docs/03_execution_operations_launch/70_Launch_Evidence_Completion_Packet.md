...
T238 evidence captured on the approved test server local API/DB. The read-only smoke selected transaction display 51001, invoice display 44001, wallet display 41001, and ledger display 50002; payment reconciliation list/detail returned matching public display evidence; daily reconciliation for 2026-04-23 returned `mismatched` with wallets checked 2, wallet mismatches 1, invoices checked 1, invoice mismatches 0, payments checked 1, and duplicate payment references 0. Before/after DB baselines were unchanged, and no money or provider mutation routes were called. Finance owner review/sign-off is still missing because the target report is not balanced.
