...
- provider-controlled redacted runtime examples for provider 5xx and cancel/delete rejected;
...
| Error examples | Redacted auth, permission, rate limit, validation, out-of-capacity, timeout, duplicate, 5xx, not-found, and cancel-rejected examples. | Partial: redacted provider edge/gateway HTTP `403` shape from the earlier blocked run, app-level unauthenticated HTTP `401`, T249 duplicate/timeout behavior, T250 auth missing, auth invalid, not-found, and malformed-validation examples, T256 provider-controlled permission-denied runtime output, T257 provider-controlled out-of-capacity runtime output, and T260 provider-controlled rate-limit runtime output are captured. T255 records the provider source-inspection and safe execution plan for the remaining cases. Provider-controlled 5xx and cancel/delete rejected runtime outputs are still missing. |
