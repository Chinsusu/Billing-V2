...
**Tasks:** T199, T208, T211, T212, T213, T214, T215, T216, T217, T218, T219, T220, T221, T226, T227, T228, T229, T230, T231
**Date:** 2026-05-17
**Decision:** real provider sandbox is not launch-ready yet. Cloudmini V3 intake, authenticated read-only reachability, guarded dev mapping, source-match fail-closed behavior, one controlled dev create/delete pilot, fail-closed non-usable provider status handling, lifecycle-worker provider cleanup, target test-server deploy/build evidence, and non-mutating Cloudmini worker registry activation are proven. Approved shared credential storage, named launch owners, duplicate/timeout live evidence, provider error examples, and owner-approved mutating/lifecycle activation are still missing before broader pilot or production-like provisioning.
...
- owner-approved mutating/lifecycle activation showing the lifecycle worker can run a bounded provider-backed cleanup with the approved Cloudmini provider registry;
- owner-approved confirmation that the fail-closed usable-status policy matches Cloudmini production semantics.
...
| Cleanup and rollback | How to list test resources, cancel/delete them, disable the provider source, and assign manual cleanup owner. | Partial: T228 cleaned up the single dev resource through provider V3 `DELETE` and marked the Billing service terminated. T229 adds provider-backed lifecycle-worker termination before service `terminated`; T230 proves the hardened code is deployed and builds on the target test server. T231 proves non-mutating worker registry activation with the real Cloudmini adapter. Owner-approved mutating/lifecycle cleanup activation and cleanup owner sign-off are still missing. |
