...
- target Finance owner sign-off for the balanced T239 reconciliation evidence, secret/key handling review, and security sign-off evidence;
...
| Retry/idempotency | Duplicate create behavior, timeout-after-send behavior, request/status lookup support, and mapping to retry safety or manual review. | Partial: repo tests cover request-known timeout/manual review and non-usable status/manual review. Live duplicate-create and timeout-after-send evidence against Cloudmini are still missing. |
