...
**Tasks:** T199, T208, T211, T212, T213, T214, T215, T216, T217, T218, T219, T220, T221, T226, T227, T228, T229, T230, T231, T232, T233
**Date:** 2026-05-17
**Decision:** real provider sandbox is not launch-ready yet. Cloudmini V3 intake, authenticated read-only reachability, guarded dev mapping, source-match fail-closed behavior, one controlled dev create/delete pilot, fail-closed non-usable provider status handling, lifecycle-worker provider cleanup code, target test-server deploy/build evidence, and non-mutating Cloudmini worker registry activation are proven. T232 reached a real dev mutating create path but was blocked before lifecycle cleanup because Cloudmini returned non-usable status `creating`; same-session provider delete cleanup succeeded. T233 adds a bounded wait/read policy for Cloudmini resources that become usable after operation success while preserving manual review for resources that stay non-usable. Approved shared credential storage, named launch owners, duplicate/timeout live evidence, provider error examples, target deploy/rerun evidence, and owner-approved lifecycle activation are still missing before broader pilot or production-like provisioning.
...
- target deploy and owner-approved lifecycle activation showing the lifecycle worker can run a bounded provider-backed cleanup with the approved Cloudmini provider registry;
...
| Cleanup and rollback | How to list test resources, cancel/delete them, disable the provider source, and assign manual cleanup owner. | Partial: T228 cleaned up the single dev resource through provider V3 `DELETE` and marked the Billing service terminated. T229 adds provider-backed lifecycle-worker termination before service `terminated`; T230 proves the hardened code is deployed and builds on the target test server. T231 proves non-mutating worker registry activation with the real Cloudmini adapter. T232 proved same-session fallback cleanup after a non-usable `creating` status. T233 adds bounded status wait code, but target lifecycle-worker cleanup activation and cleanup owner sign-off are still missing. |
| Pilot run | Redacted evidence for one approved sandbox order through checkout, reservation, provider request, service activation, credential storage/reveal audit, and cleanup. | Partial/pass for one dev pilot: T228 created one order/job/service through Billing path, stored encrypted credential metadata, and completed same-session provider cleanup. T232 ran one additional dev create attempt and cleanup, but it did not activate a service because provider status remained `creating`. T233 adds code to wait for usable status before service activation. Duplicate create, timeout-after-send, credential reveal audit, lifecycle-worker cleanup activation, and broader owner sign-off remain missing. |
