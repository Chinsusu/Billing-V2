...
- Error evidence status: T250 adds `go run ./cmd/smoke cloudmini-error-evidence` for owner-approved non-production Cloudmini error evidence. The approved target dev/test run returned `PASS` with four redacted examples: missing auth `401` mapped to `PROVIDER_AUTH_FAILED`; invalid auth `401` mapped to `PROVIDER_AUTH_FAILED`; a valid-format missing proxy UUID returned `404 PROXY_NOT_FOUND` mapped to `PROVIDER_STATE_DRIFT` with `manual_review_required`; malformed create JSON returned `400 INVALID_INPUT` mapped to `PROVIDER_CONFIG_INVALID`. The malformed create case was explicitly approved, called one mutating route with invalid JSON, and did not print or record any raw body, token, provider ID, provider payload, proxy credential, cookie, or file contents. T255 records source-inspection evidence and a safe execution plan for the remaining provider-controlled cases in `docs/03_execution_operations_launch/77_Cloudmini_Provider_Controlled_Error_Evidence.md`. T256 extends the smoke with guarded permission-denied collection and the approved dev/test run returned `403`, `PROVIDER_PERMISSION_DENIED`, `do_not_retry`, temporary API key revoked, active key count restored, and `mutating_routes_called=true` for the API-key create/revoke support routes. T257 extends the smoke with guarded out-of-capacity collection and the approved dev/test run returned `409 CAPACITY_EXHAUSTED`, `PROVIDER_OUT_OF_STOCK`, `do_not_retry`, one exhausted-group reservation probe, and `reservation_created=false`. T258 extends the smoke with guarded rate-limit fixture support and local coverage for `429 RATE_LIMITED` to `PROVIDER_RATE_LIMITED` with `safe_retry`. T259 merges the provider-side rate-limit fixture code in Cloudmini PR #9. T260 ran the approved dev/test rate-limit fixture evidence and returned `429 RATE_LIMITED`, `PROVIDER_RATE_LIMITED`, `safe_retry`, and `mutating_routes_called=false`; the fixture env was removed after the run. T261 ran the approved dev/test provider 5xx fixture evidence and returned `500 INTERNAL_ERROR`, `PROVIDER_TEMPORARY_ERROR`, `safe_retry`, and `mutating_routes_called=false`; the fixture env was removed after the run. T262 ran the approved dev/test cancel/delete rejected fixture evidence and returned failed operation state `failed`, `DELETE_FAILED`, `PROVIDER_PARTIAL_SUCCESS`, `manual_review_required`, and `mutating_routes_called=false`; the fixture env was removed after the run. The listed provider-controlled runtime examples are closed for the selected dev/test evidence scope.
- Provider-owner approval status: T263 records `Admin` as sandbox account owner, provider support contact, provider owner, engineering lead, ops lead, security owner, cleanup owner, and final reviewer for the selected bounded Cloudmini non-production pilot. Admin approves the current edge/header policy proven by the Billing Go-client path, the `proxy-static-10gb-monthly` to redacted `ipv4_dc` group mapping, the one-active-resource quota, no parallel mutating calls, same-session cleanup, and stop-on-unexpected-success or cleanup-failure conditions. This approval does not cover production customer data, additional provider sources, additional Cloudmini accounts, unbounded provisioning, or a different secret path without new evidence.
...
T261 approved dev/test provider 5xx fixture evidence redacted stdout excerpt:
...
T262 approved dev/test cancel/delete rejected fixture evidence redacted stdout excerpt:
...
example_4_error_envelope_present=false
example_4_error_message_field_present=true
example_4_error_details_field_present=false
example_4_side_effect_created=not_applicable
...
Still missing before expanding beyond the selected bounded Cloudmini scope:
...
- owner-approved timeout/spend guardrail sign-off for any limit above one active non-production resource;
- new provider-owner approval for production customer data, additional Cloudmini accounts, additional secret paths, or production-like provisioning beyond the selected pilot semantics.

## Evidence Packet Status

One approved dev mutating pilot evidence packet is stored in this repository as of 2026-05-17. T263 records the owner approval needed to treat the selected bounded Cloudmini non-production scope as provider-ready; anything outside that scope still needs a new packet.

| Evidence area | Required proof | Current repo status |
|---|---|---|
| Provider intake | Provider name, sandbox account owner, support contact, docs/API version, and sandbox base URL. | Pass for selected scope: Cloudmini V3, API V3, and `https://cz.resvn.net/` are recorded; Admin is sandbox account owner, provider owner, provider support contact, and final reviewer. |
...
| Quota and cost guardrail | Sandbox quota, rate/concurrency limits, maximum spend or credit exposure, and stop condition. | Pass for selected scope: Admin approves one active non-production resource, one worker concurrency, no parallel mutating calls, single-dev-resource exposure, and stop on unexpected success, cleanup failure, credential exposure, duplicate resource, provider account issue, or spend/quota ambiguity. |
| Capability mapping | Product type, Billing plan code, provider SKU, location, inventory mode, auto/manual provisioning support, cancellation support, and credential retrieval behavior. | Pass for selected scope: authenticated read-only inventory succeeds, `ipv4_dc` has sellable capacity, `residential` remains exhausted/not approved, and Admin approves `proxy-static-10gb-monthly` mapped to the redacted Cloudmini `ipv4_dc` group with cleanup/cancellation through provider `DELETE`. |
| Retry/idempotency | Duplicate create behavior, timeout-after-send behavior, request/status lookup support, and mapping to retry safety or manual review. | Pass for selected scope: repo tests cover request-known timeout/manual review and non-usable status/manual review. T249 ran duplicate-create against Cloudmini with two create attempts, one distinct redacted resource, duplicate protection, and cleanup success. T249 ran timeout-after-send with `PROVIDER_TIMEOUT_REQUEST_KNOWN`, `manual_review_required`, one redacted resource, and cleanup success. |
| Error examples | Redacted auth, permission, rate limit, validation, out-of-capacity, timeout, duplicate, 5xx, not-found, and cancel-rejected examples. | Pass for selected dev/test error evidence: redacted provider edge/gateway HTTP `403` shape from the earlier blocked run, app-level unauthenticated HTTP `401`, T249 duplicate/timeout behavior, T250 auth missing, auth invalid, not-found, and malformed-validation examples, T256 provider-controlled permission-denied runtime output, T257 provider-controlled out-of-capacity runtime output, T260 provider-controlled rate-limit runtime output, T261 provider-controlled 5xx runtime output, and T262 cancel/delete rejected runtime output are captured. |
...
| Pilot run | Redacted evidence for one approved sandbox order through checkout, reservation, provider request, service activation, credential storage/reveal audit, and cleanup. | Pass for selected scope: T228 created one order/job/service through Billing path, stored encrypted credential metadata, and completed same-session provider cleanup. T233 target rerun created one additional order/job/service, stored one encrypted active credential, and completed lifecycle-worker provider cleanup. T235 proved target top-up review E2E using a temporary dev/test wallet: approve posted one ledger credit and audit row, reject posted no ledger and one audit row, and provider side effects were `none`. T237 proved target credential reveal audit/redaction on a dev/test fixture without provider or money mutation routes. T238 proved target finance reconciliation read paths and T239 resolved the dev/test projection mismatch with audit display `10018`; the target finance smoke now returns `balanced`. T249 proved duplicate-create and timeout-after-send provider behavior with same-session cleanup. T263 records Admin owner sign-off for this selected provider scope. |
...
Before expanding beyond the selected bounded non-production Cloudmini scope, record the required provider intake from `docs/05_development_standards/60_Provider_Sandbox_Contract_Checklist.md`:
...
- provider-owner approval for the expanded account, quota, mapping, and production-like provisioning scope;
- cleanup and rollback plan for any future resources created outside the selected pilot evidence scope.

If any item is missing, keep the expanded provider scope blocked.
