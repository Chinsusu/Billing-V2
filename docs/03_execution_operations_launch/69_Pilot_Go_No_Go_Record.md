...
| `docs/03_execution_operations_launch/66_Provider_Sandbox_Readiness_Evidence.md` | blocked for broader pilot | Cloudmini read-only, dev mapping, one controlled dev create/delete pilot, repo-side non-usable-status handling, and lifecycle-worker provider cleanup are proven. Shared secret storage, named owners, live duplicate/timeout evidence, redacted error examples, and target-environment evidence remain missing. |
...
| Checkout debit/reservation/provisioning flow | pass for fake provider and one controlled dev Cloudmini pilot | T196 reservation concurrency; T204 wallet payment, provisioning job, worker fulfillment, audit checks, and active service verification passed for fake provider. T228 passed one Cloudmini dev Billing-path create with encrypted credential storage and same-session cleanup. T229 prevents Cloudmini non-usable statuses from creating active services. | Broader real provider pilot remains blocked by doc 66 residual risks: shared secret storage, named owners, live duplicate/timeout evidence, target-environment lifecycle-worker cleanup evidence, and owner sign-off. |
