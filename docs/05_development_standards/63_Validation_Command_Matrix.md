...
`cloudmini-error-evidence` requires owner-approved non-production Cloudmini credentials and explicit approval env. It captures redacted error metadata only: auth missing, invalid auth, not-found, and optional malformed-create validation. The malformed-create validation uses an invalid JSON body and must be enabled with `CLOUDMINI_ERROR_EVIDENCE_ALLOW_INVALID_CREATE=yes`, `CLOUDMINI_ERROR_EVIDENCE_MUTATING_ROUTE_APPROVED=yes`, and `CLOUDMINI_ERROR_EVIDENCE_MAX_CREATE_ATTEMPTS=1`. It must not print raw response bodies, tokens, provider IDs, provider payloads, proxy credentials, cookies, or file contents. It must never point at production or real customer data.
