...
| Target auth/RBAC smoke | `make smoke-dev-target-auth-rbac` | `go run ./cmd/smoke dev-target-auth-rbac` | Target auth session, 2FA gate, RBAC denial, cross-tenant denial, or public-domain target auth evidence changed. |
...
`dev-target-auth-rbac` requires the API and database to point at the same approved dev/test environment. It creates dev/test auth sessions, verifies cookie-only client access, 2FA admin blocking, invalid session denial, missing actor denial, cross-tenant denial, and RBAC permission denial. It must never point at production. Use `-base-url` for a single local API base URL. For public-domain target checks where client and platform admin login resolve from different hostnames, use `-target-auth-client-base-url` and `-target-auth-admin-base-url`, or set `BILLING_TARGET_AUTH_SMOKE_CLIENT_BASE_URL` and `BILLING_TARGET_AUTH_SMOKE_ADMIN_BASE_URL`; both default to `-base-url` when omitted. When the selected target credentials differ from the original dev seed defaults, set `BILLING_TARGET_AUTH_SMOKE_CLIENT_EMAIL`, `BILLING_TARGET_AUTH_SMOKE_CLIENT_PASSWORD`, `BILLING_TARGET_AUTH_SMOKE_ADMIN_EMAIL`, and `BILLING_TARGET_AUTH_SMOKE_ADMIN_PASSWORD` from the protected non-repo secret store before running the smoke; do not print or commit the values.
