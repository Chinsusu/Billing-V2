...
Use UUID/ULID-style public IDs for externally referenced resources.

```text
Avoid exposing sequential integer IDs in public APIs.
Use internal bigint only if hidden behind public_id.
```
...
```text
id UUID PRIMARY KEY
public_id TEXT UNIQUE optional
