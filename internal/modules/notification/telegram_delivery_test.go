...
func TestTelegramDeliveryHandlerMarksHTTPFailureTerminal(t *testing.T) {
	now := time.Date(2026, 5, 19, 16, 0, 0, 0, time.UTC)
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if completion.Status != jobs.StatusFailedTerminal || completion.RetrySafety != jobs.RetrySafetyDoNotRetry {
		t.Fatalf("unexpected terminal completion: %+v", completion)
	}
	if store.failedID != "44444444-4444-4444-4444-444444444444" ||
		store.failedErrorCode != "telegram_http_400" ||
		strings.Contains(store.failedErrorMessage, "do not copy") {
		t.Fatalf("unexpected failed store state: %+v", store)
	}
}
