    ...
    <div className="p-5">
      <div className="bg-white border border-gray-200 rounded">
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
          <h3 className="text-[13px] font-semibold text-gray-900 m-0">Support tickets</h3>
          ...
          <tbody>
            {TICKETS.map((t) => (
              <tr key={t.id} className="hover:bg-gray-50 border-b border-gray-100 last:border-0">
                ...
                <td className="px-3 py-2 text-gray-800 max-w-[300px] truncate">{t.subject}</td>
                <td className="px-3 py-2 text-gray-500">{t.customer}</td>
                <td className={`px-3 py-2 text-[12px] capitalize ${PRIORITY_CLASS[t.priority]}`}>{t.priority}</td>
                <td className="px-3 py-2"><StatusBadge status={t.status} dot /></td>
                <td className="px-3 py-2 text-gray-400">{t.updated}</td>
