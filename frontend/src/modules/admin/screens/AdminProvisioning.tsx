  ...
  return (
    <div className="p-4 flex flex-col gap-4">
      {manualReview.length > 0 && (
        <div className="bg-amber-50 border border-amber-200 text-amber-700 text-[12px] p-4 p-4.5 rounded flex items-center gap-4">
          <span>⚠</span>
          ...
          <tbody>
            {PROVISIONING_JOBS.map((job) => (
              <tr key={job.id} className={`hover:bg-gray-50 border-b border-gray-100 last:border-0 ${job.status === "manual_review" ? "bg-amber-50/40" : ""}`}>
                <td className="p-4 p-4 text-[12px] text-gray-500">{job.id}</td>
                <td className="p-4 p-4 text-[12px] text-[#D50C2D]">{job.order}</td>
                <td className="p-4 p-4 text-gray-700">{job.service}</td>
                <td className="p-4 p-4 text-gray-500">{job.tenant}</td>
                <td className="p-4 p-4 text-gray-500">{job.provider}</td>
                <td className="p-4 p-4"><StatusBadge status={job.status} dot /></td>
                <td className="p-4 p-4 text-center tabular-nums">{job.attempt}</td>
                <td className="p-4 p-4 text-gray-400 tabular-nums">{job.age}</td>
                <td className="p-4 p-4 text-[11px] text-red-600 max-w-[200px] truncate">{job.error || "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
