          ...
          <tbody>
            {PRODUCTS.map((p) => (
              <tr key={p.sku} className="hover:bg-gray-50 border-b border-gray-100 last:border-0">
                <td className="p-4 p-4 text-[12px] text-gray-500">{p.sku}</td>
                <td className="p-4 p-4 font-medium text-gray-900">{p.name}</td>
                <td className="p-4 p-4 text-gray-400 text-[12px]">{p.unit}</td>
                <td className="p-4 p-4 tabular-nums font-medium">{fmtMoney(p.price)}</td>
                <td className="p-4 p-4 tabular-nums text-right">{p.active.toLocaleString()}</td>
                <td className="p-4 p-4 tabular-nums text-right font-medium">{fmtMoneyShort(p.rev30)}</td>
              </tr>
            ))}
          </tbody>
        </table>
