    ...
    <div className="p-5">
      <div className="bg-white border border-gray-200 rounded">
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
          <h3 className="text-[13px] font-semibold text-gray-900 m-0">Customers</h3>
          ...
          <tbody>
            {CUSTOMERS.map((c) => (
              <tr key={c.id} className="hover:bg-gray-50 border-b border-gray-100 last:border-0">
                ...
                <td className="px-3 py-2 tabular-nums text-right font-medium">{fmtMoneyShort(c.mrr)}</td>
                <td className="px-3 py-2"><StatusBadge status={c.status} dot /></td>
                <td className="px-3 py-2 text-gray-400">{c.since}</td>
