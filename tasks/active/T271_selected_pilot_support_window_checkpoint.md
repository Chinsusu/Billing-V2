# T271 - Selected pilot support-window checkpoint

Status: DONE
Owner: Codex
Branch: codex/t271-support-window-checkpoint
PR: https://github.com/Chinsusu/Billing-V2/pull/573
...
Record a selected bounded non-production pilot support-window checkpoint before final closeout.

## Scope

- Verify selected runtime services and public domains during the support window.
...
- Record that this is an in-window checkpoint, not the final support-window closeout.
...
- Docs clearly state final closeout remains due after `2026-05-19 22:00 Asia/Ho_Chi_Minh`.
- Task board remains consistent and required checks pass.

## Notes

- This task cannot truthfully mark the support window closed because it starts before `2026-05-19 22:00 Asia/Ho_Chi_Minh`.
...
- 2026-05-19: Task created and claimed by Codex from Billing `origin/main` at `18:53 Asia/Ho_Chi_Minh`; final support-window closeout remains due after `22:00 Asia/Ho_Chi_Minh`.
- 2026-05-19: Runtime checks passed at `18:55 Asia/Ho_Chi_Minh`: `billing-api`, `billing-frontend`, and `cloudflared` active/enabled with main PIDs present.
...
- 2026-05-19: Updated docs 69 and 70 with selected support-window checkpoint evidence and explicit note that final closeout remains due after `22:00 Asia/Ho_Chi_Minh`.
- 2026-05-19: Opened PR #573. Local checks passed: service/domain health, command-line secret-pattern checks, protected metadata checks, finance reconciliation smoke, read-only notification count query, `go run ./cmd/taskguard`, `git diff --check`, raw-secret pattern scan, and changed-file line-count check.
- 2026-05-19: PR #573 merged after GitHub checks passed. Marking task DONE in marker PR.
